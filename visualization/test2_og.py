...
step_size = 1  # Step size towards the sampled point
...
def find_nearest(tree, sample, goal):
    ...
    for node in tree:
        ...
        if dist < min_dist:
            min_dist = dist
            closest_node = node 
    if min_dist > goal_dist and goal_dist != 0:
        min_dist = goal_dist
        closest_node = goal
...
def rrt(start, goal, map_file):
    ...
    for iteration in range(max_iterations):
        ...
        goal_dist = dist_to_goal(new_node, goal)
...
def plot_and_connect(points, max_distance):
    ...
    # Find the closest node to the last node
    closest_node = min(points[:-1], key=lambda x: distance(x, points[-1]))
    
    # Connect the last node to the closest node
    ax.plot([points[-1][0], closest_node[0]], [points[-1][1], closest_node[1]], 'k-')
