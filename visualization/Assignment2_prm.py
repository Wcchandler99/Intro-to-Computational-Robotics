...
def construct_prm_graph(nodes, obstacles, robot_type, k=6):
    ...
    for i, node in enumerate(nodes):
        ...
        for neighbor_index, _ in distances[:k]:
            ...
            if is_path_collision_free(node, neighbor, obstacles, robot_type):
                graph['edges'].append((i, neighbor_index))
    return graph
...
def find_path_prm(start_config, goal_config, graph, robot_type):
...
def visualize_arm_movement(path, obstacles):
    ...
    viz_group.to_html("prm_arm_movement_animation.html", "../out/")
...
def visualize_freeBody_path(path, obstacles):
    ...
    viz_out.to_html("prm_vehicle_movement_animation.html", "../out/")

def visualize_prm(prm_graph, obstacles, filename="prm_with_obstacles_visualization.html"):
    """
...
def main_prm():
    ...
    obstacles = load_obstacles(args.map)  # Load obstacles from file
    visualize_obstacles(obstacles)  # Visualize the obstacles
    nodes = generate_prm_nodes(args.robot, 5000, obstacles)  # Generate PRM nodes
    graph = construct_prm_graph(nodes, obstacles, args.robot, k=6)  # Construct the PRM graph
    # Convert start and goal configurations to the appropriate format if necessary
    start_config = np.array(args.start)
    goal_config = np.array(args.goal)
    path = find_path_prm(start_config, goal_config, graph, args.robot)  # Find path using PRM
    ...
    if path:
        ...
        if args.robot == 'arm':
            # Visualize path for arm robot
            visualize_arm_movement(path, obstacles)
        elif args.robot == 'freeBody':
            # Visualize path for freeBody robot
            visualize_freeBody_path(path, obstacles)
        # Visualize the PRM alongside obstacles
        visualize_prm(graph, obstacles, "prm_with_obstacles_visualization.html")
        visualize_prm_path(path, obstacles, "prm_path_visualization.html", args.robot)
    else:
        print("No path found.")
